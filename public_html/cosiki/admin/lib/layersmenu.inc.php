...
//tutaj dodatki RS - na potrzeby programu SKLEP
		if (isset($id_zad))
		 { $buffer = ereg_replace("id_zad", $id_zad, $buffer);
		 }
		else
		 { $buffer = ereg_replace("id_zad", "0", $buffer);
		 }
		if (isset($pozycja))
		 { $buffer = ereg_replace("pozycja", $pozycja, $buffer);
		 }
		else
		 { $buffer = ereg_replace("pozycja", "0", $buffer);
		 }
//koniec dodtaków RS
